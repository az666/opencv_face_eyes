...
def detectFaces(image_name):
    ...
    face_cascade = cv2.CascadeClassifier("E:/HelloWord/opencv/opencv-master/data/lbpcascades/lbpcascade_frontalface_improved.xml")
    if img.ndim == 3:
...
def detectEyes(image_name):
    eye_cascade = cv2.CascadeClassifier('E:/HelloWord/opencv/opencv-master/data/haarcascades/haarcascade_eye.xml')
    faces = detectFaces(image_name)
    img = cv2.imread(image_name)
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    result = []
...
def detectSmiles(image_name):
    img = cv2.imread(image_name)
    smiles_cascade = cv2.CascadeClassifier("E:/HelloWord/opencv/opencv-master/data/haarcascades/haarcascade_smile.xml")
    if img.ndim == 3:
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    else:
        gray = img #if语句：如果img维度为3，说明不是灰度图，先转化为灰度图gray，如果不为3，也就是2，原图就是灰度图
...
def drawSmiles(image_name):
    ...
    if smiles:
        ...
        #mg.save('drawsmiles_'+image_name)




# drawFaces('E:/opencv_pictures/obama.jpg')

if __name__ == '__main__':
   # drawFaces('E:/opencv_pictures/obama.jpg')
    #drawEyes('E:/opencv_pictures/obama.jpg')
    drawSmiles('E:/opencv_pictures/obama.jpg')
    #saveFaces('E:/opencv_pictures/obama.jpg')
